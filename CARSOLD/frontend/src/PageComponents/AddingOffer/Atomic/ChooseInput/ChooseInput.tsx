    ...
    return (
        <div className="relative w-fit">
            ...
            <div className="flex flex-row text-lg m:text-xl">
                <button className={`w-[140px] m:w-[150px] h-12 rounded-l-md border
                ...
                <button className={`w-[140px] m:w-[150px] h-12 rounded-r-md border
                ${error ? "border-coolRed bg-white text-coolRed" 
                    : value === secondOption ? "border-darkLime bg-lime" : "border-gray-300 bg-white" }`}
                        onClick={() => handleButtonClick(secondOption)}>{secondOption}
                </button>
            </div>
            {required && <FontAwesomeIcon className="absolute -right-3 m:-right-3.5 top-[50px] text-[10px] m:text-xs pointer-events-none" icon={faAsterisk} />}
