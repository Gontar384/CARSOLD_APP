    ...
    return (
        <div className={`flex flex-col ${isWide ? "justify-start gap-6 lg:gap-8 xl:gap-10 2xl:gap-12 3xl:gap-14"
            ...
            <div className={`flex ${isWide ? "flex-col gap-6 lg:gap-8 xl:gap-10 2xl:gap-12 3xl:gap-14"
                ...
                <ChoiceButton label={"My Offers"} serial={0} onClick={() => handleNavigation("myOffers")} active={buttonState[0]}/>
                <ChoiceButton label={"Followed"} serial={1} onClick={() => handleNavigation("followed")} active={buttonState[1]}/>
                <ChoiceButton label={"Messages"} serial={2} onClick={() => handleNavigation("messages")} active={buttonState[2]}/>
            </div>
            <div className={`flex ${isWide ? "flex-col gap-6 lg:gap-8 xl:gap-10 2xl:gap-12 3xl:gap-14"
                : "flex-row justify-evenly mx-[50px] xs:mx-[72px]"}`}>
                <ChoiceButton label={"Settings"} serial={3} onClick={() => handleNavigation("settings")} active={buttonState[3]}/>
                <ChoiceButton label={"Info"} serial={4} onClick={() => handleNavigation("info")} active={buttonState[4]}/>
            </div>
        </div>
    )
}
