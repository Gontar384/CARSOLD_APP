...
const ChoiceButton: React.FC<ChoiceButtonProps> = ({ label, onClick, active }) => {
    ...
    return (
        <button className={'flex justify-center items-center py-[2px] px-[6px] w-[95px] xs:w-32 lg:w-[138px] xl:w-[146px] 2xl:w-[164px] 3xl:w-[186px] ' +
                ...
                ` text-${buttonColor ? "white" : "black"} text-[17px] xs:text-lg lg:text-xl xl:text-[22px] 2xl:text-[26px] 3xl:text-3xl
                ${buttonColor ? "ring-[3px] xs:ring-4 2xl:ring-[5px] 3xl:ring-[6px] ring-lowLime/80 animate-gentle" : ""}
                ${active && !buttonColor ? "border-blue-500/30 border-2 xs:border-[3px] 2xl:border-4" : ""}`}
            onClick={onClick}
