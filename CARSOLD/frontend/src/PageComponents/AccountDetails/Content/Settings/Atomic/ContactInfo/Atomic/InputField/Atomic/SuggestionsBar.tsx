...
const SuggestionsBar: React.FC<SuggestionsBarProps> = ({citySuggestions, setCitySuggestions, setValue, setClickedSuggestion}) => {
    ...
    return (
        <div>{citySuggestions && citySuggestions.slice(0, 4).map((citySuggestion: string, index: number ) => (
                ...
                text-left bg-white border-y border-black border-opacity-40 whitespace-nowrap overflow-hidden"
                        title={citySuggestions[index]}
                        onClick={() => handleClickSuggestion(index)}>{citySuggestion}</button>
            ))}
        </div>
    )
}
