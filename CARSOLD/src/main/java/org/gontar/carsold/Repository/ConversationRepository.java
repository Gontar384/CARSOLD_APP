...
public interface ConversationRepository  extends JpaRepository<Conversation, Long> {
    ...
    @Query("SELECT COUNT(c) FROM Conversation c WHERE c.user1.id = :userId AND c.seenByUser1 = false AND c.activatedByUser1 = true AND c.deletedByUser1 = false")
    int countUnseenAndActivatedByUser1(@Param("userId") Long userId);
    @Query("SELECT COUNT(c) FROM Conversation c WHERE c.user2.id = :userId AND c.seenByUser2 = false AND c.activatedByUser2 = true AND c.deletedByUser2 = false")
